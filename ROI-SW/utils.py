...
norm_info = {
    ...
    "0.8": {
        "centre": {
            "all_mean": [[0.03567150591730152, 0.10256677866767831], [0.03931357978822218, 0.10389137797587274],
                         [0.046538337850875175, 0.10413719653644818], [0.03822977378247555, 0.10337194824526984],
                         [0.035812401883830454, 0.10594183085632256]],
            "all_deviation": [[1.1330233260597609, 0.09871420850175948], [1.1778767230062088, 0.09751525687408301],
                              [1.1518327058931004, 0.0970320179665146], [1.1230847455864355, 0.09802497365383343],
                              [1.16066576592683, 0.09548291707927135]]
        },
        "left_boundary": {
            # to be filled
        }
        ...
        for pred_index in range(len(pred_cls)):
            if pred_cls[pred_index] in gt_cls:  # ground truth is true
                ...
                if max([gt_cls.count(item) for item in gt_cls]) > 1 and len(
                    ...
                    wait = True
                if th_iou != 1:
                    threshold = th_iou if compute_phrase_length(pred_bboxes[pred_index]) > 5 else 0.4
                else:
                    threshold = th_iou
