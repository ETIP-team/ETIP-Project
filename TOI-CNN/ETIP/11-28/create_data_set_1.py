...
path = "dataset/train/train_relabeled_data_11_22/"
pkl_path = "dataset/train/train_relabeled_data_pkl_11_22/"

# path = "dataset/test/test_relabeled_data_11_22/"
# pkl_path = "dataset/test/test_relabeled_data_pkl_11_22/"
...
for file in os.listdir(path):
    ...
    ls_data = text.split("\n\n")
    for index in range(len(ls_data)):
        if ls_data[index].count("\n") > 1:
            wait = True
        ls_data[index] = [item.strip(" ").strip("|") for item in ls_data[index].split("\n")]
    ...
    for sub_ls in ls_data:
        ...
        if len(sub_ls) < 1:
            continue
        one_data = {}
        one_data["sentence"] = str2wv(sub_ls[0].split(" ")).reshape(1, cfg.SENTENCE_LENGTH, word_embedding_dim)#72*300
        one_data["str"] = sub_ls[0]
        # print(one_data["sentence"].shape)
        # exit()
        sentence_length = len(sub_ls[0].split(" "))
