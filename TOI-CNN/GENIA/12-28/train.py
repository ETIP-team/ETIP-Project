...
def train_batch(sentence, rois, ridx, gt_cls, gt_target_bbox, rcnn):
    predict_cls, predict_target_bbox = rcnn(sentence, rois, ridx)
    loss, loss_cls, loss_loc = rcnn.calc_loss(predict_cls, predict_target_bbox, gt_cls, gt_target_bbox)
...
def train_epoch(train_arguments, rcnn):
    ...
    for sentence, rois, ridx, gt_cls, gt_target_bbox in train_arguments.batch_train_data_generator(perm):
        loss, loss_cls, loss_loc = train_batch(sentence, rois, ridx, gt_cls, gt_target_bbox, rcnn)
        losses.append(loss)
...
def train(train_arguments):
    ...
    if train_arguments.normalize:
        if train_arguments.dx_compute_method == "left_boundary":
            train_arguments.train_target_bbox = npz["train_norm_lb_tbbox"]
        else:
            train_arguments.train_target_bbox = npz["train_norm_tbbox"]
    else:
        train_arguments.train_target_bbox = npz['train_tbbox']
    ...
    for epoch_time in range(train_arguments.max_iter_epoch):
        ...
        if epoch_time >= train_arguments.start_save_epoch:
            save_directory = train_arguments.get_model_path()
...
def train_k_fold():
    ...
    pos_loss_type = "mse"  # "mse"   #  "smoothL1"
    prevent_overfitting_method = "Dropout"  # "L2 Regu"  # "Dropout"
    dropout_rate = 0.5
    dx_compute_method = "centre"  # history method:   use left boundary to represent the dx.

    loss_weight_lambda = 0
    norm = True  # False
